...
// import {AS} from "../src/AS.sol";
import {WeightsAlloc} from "../src/lib/WeightsAlloc.sol";
import {FixedBase} from "../src/lib/utils/FixedBase.sol";
...
contract ASTest is ASCommon {
    ...
    uint8 constant W_FRAC = 8; // Q8.8
    uint256 constant W_SCALE = 1 << W_FRAC; // 2^8
    ...
    function test_DataStorage() public {
        InitData memory _d;
        mockInit(_d);
        emit log_named_array("Strategies", convertArrUint8toUint256(AS_STORAGE.getEnabledStrategies()));
        emit log_named_array("Categories", convertArrUint8toUint256(AS_STORAGE.getEnabledCategories()));

        _d.eIds = new uint32[](5);
        _d.eIds[0] = 11;
        _d.eIds[1] = 22;
        _d.eIds[2] = 33;
        _d.eIds[3] = 44;
        ...
        // AS_STORAGE.disableCategory(_d.cIds[1]);
        // AS_STORAGE.disableCategory(_d.cIds[2]);

        AS_STORAGE.addEntities(_d.eIds, _d.cIds, newVals);

        for (uint8 i = 0; i < _d.eIds.length; i++) {
            uint32 eId = _d.eIds[i];
            uint16[] memory vals = AS_STORAGE.getCategoryValues(eId);
            emit log_named_array(
                string(abi.encodePacked("Category values for entity ", vm.toString(eId))),
                convertArrUint16toUint256(vals)
            );
        }
        WeightsAlloc.Strategy memory s = AS_STORAGE._getStrategyRaw(_d.sIds[0]);
        emit log_named_array(
            string(abi.encodePacked("[debug] sumX for categories ")), convertArrFixed16ToUint256(s.sumX)
        );
        ...
        for (uint8 i = 0; i < _d.eIds.length; i++) {
            ...
            uint256 share = AS_STORAGE.shareOf(eId, _d.sIds[0]).mulN(1e18, 32);
        ...
        emit log_named_array(string(abi.encodePacked("shares for entities Q32.32")), shares);
    }
    ...
    function mockInit(InitData memory _d) internal returns (InitData memory) {
        ...
        // add 1st strategy "deposit"
        AS_STORAGE.enableStrategy(
            _d.sIds[0], WeightsAlloc.StrategyMetadata({id: _d.sIds[0], owner: address(0), description: "Deposit"})
        );
        ...
        weights[2] = 20000;
        //  (1 - DVT, 0 - other)
        AS_STORAGE.enableCategory(
            _d.cIds[0],
            weights[0],
            WeightsAlloc.CategoryMetadata({id: _d.cIds[0], owner: address(0), description: "Fee"})
        );
        AS_STORAGE.enableCategory(
            _d.cIds[1],
            weights[1],
            WeightsAlloc.CategoryMetadata({id: _d.cIds[1], owner: address(0), description: "Technology"})
        );
        AS_STORAGE.enableCategory(
            _d.cIds[2],
            weights[2],
            WeightsAlloc.CategoryMetadata({id: _d.cIds[2], owner: address(0), description: "Performance"})
        );

        // add 2nd strategy "withdrawal"
        AS_STORAGE.enableStrategy(
            _d.sIds[1], WeightsAlloc.StrategyMetadata({id: _d.sIds[1], owner: address(0), description: "Withdrawal"})
        );
